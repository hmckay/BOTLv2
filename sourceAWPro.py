    ...
    for idx in df.index:
        ...
        #continue with repro
            ...
            if weightType == 'OLSCL2' or 'OLSPAC' in weightType or 'OLSKPAC' in weightType:
                ...
                stableLocals = None
            if 'PA' in weightType:# or 'OLSKPAC' in weightType:
            ...
            if retrainFlag==False:
                ...
                if len(window) >= MAX_WINDOW:
                    ...
                    modelID, existingModels,transitionMatrix,modelOrder,targetModel,newTarget = modelMultiHistory.nextModels(
                            ...
                            weightType,tempModel,PCAVAR,DEFAULT_PRED)
                    if 'AWE' in weightType:
                    ...
                    modelID,existingModels,transitionMatrix,modelOrder,model,newTarget = modelMultiHistory.tempModel(existingModels,transitionMatrix,
                            modelOrder,window,modelID,tLabel,DROP_FIELDS,startIDX,False,tempModel,DEFAULT_PRED)
                    if 'AWE' in weightType:
                        modelSet = AWEaddToModelSet(window,lastModID,existingModels,sourceModels,modelSet,tLabel,DROP_FIELDS)
                    elif 'AddExpO' in weightType:
                        print(lastModID)
                        print(modelSet)
                        modelSet,orderedModels,weights['metaModel'] = AddExpOaddToModelSet(window,idx,lastModID,existingModels,
                                sourceModels,modelSet,orderedModels,weights,tLabel,DROP_FIELDS)
                        METASTATS['metaWeightUpdate']+=1
                        print("after")
                        print(modelSet)
                        ...
                        print(lastModID)
                        print(modelSet)
                        modelSet,orderedModels,weights['metaModel'] = AddExpPaddToModelSet(window,idx,lastModID,existingModels,
                                sourceModels,modelSet,orderedModels,weights,tLabel,DROP_FIELDS)
                        METASTATS['metaWeightUpdate']+=1
                        print("after")
                        print(modelSet)
                ...
                if n1 >= MAX_WINDOW:
                    ...
                    if modelMultiHistory.reuseModels(existingModels,window, tLabel,DROP_FIELDS):
                        ...
                        if n1 >= MAX_WINDOW/2:
                            ...
                            sourceModels,METASTATS=readyToReceive(s,sourceModels,ID,METASTATS)
                            #############NED TO FIX\\\\\\\\\\\\\\\\
                            modelID, existingModels,transitionMatrix,modelOrder,targetModel,newTarget = modelMultiHistory.tempModel(
                                    existingModels,transitionMatrix,modelOrder,window,modelID,tLabel,DROP_FIELDS,startIDX,
                                    False,tempModel,DEFAULT_PRED)
                            # weights = modelMulti.calcWeights(window,sourceModels,targetModel,tLabel,
                                    # DROP_FIELDS,weightType,CULLTHRESH,MITHRESH,stableLocals,modelID,existingModels[modelID]['PCs'])
                            # weights,distanceMatrix,affinityMatrix,groupedNames = modelMulti.calcWeights(window,sourceModels,
                                    # targetModel,tLabel,DROP_FIELDS,weightType,CULLTHRESH,MITHRESH,stableLocals,modelID,

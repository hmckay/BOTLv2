...
def calcWeights(df,sourceModels,targetModel,tLabel,DROP_FIELDS,weightType,ct,mi,stableLocals=None,modelID=None,
        PCs=None,distanceMatrix=None,affinityMatrix=None,newTarget=False,groupedNames=None,metaModelKeep=None):
    global CULLTHRESH 
    global MITHRESH
    CULLTHRESH = ct
    MITHRESH = mi
    if weightType =='R2':
        return calcR2Weights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS)
    elif weightType =='OLS' or weightType == 'OLSPARed':
        return calcOLSWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS)
    elif weightType =='Ridge':
        return calcRidgeWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS)
    elif weightType =='NNLS':
        return calcNNLSWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS)
    elif weightType =='OLSFE' or weightType == 'OLSFEPA':
        return calcOLSFEWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS,distanceMatrix)
    elif weightType =='OLSFEMI':
        return calcOLSFEMIWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS)
    elif weightType =='OLSCL':
        return calcOLSCLWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS)
    elif weightType =='OLSCL2':
        return calcOLSCL2Weights(df,sourceModels,targetModel,tLabel,DROP_FIELDS,stableLocals,modelID,None,
        ...
        return calcOLSCDOEWeights(df,sourceModels,targetModel,tLabel,DROP_FIELDS,stableLocals,modelID,PCs,
                distanceMatrix,affinityMatrix,newTarget,groupedNames,metaModelKeep)
    ...
    # elif weightType == 'OLSAddExpP':
        ...
        return calcOLSAWEWeights(df,sourceModels,targetModel,tLabel,DROP_FIELDS,stableLocals,modelID,PCs,
                distanceMatrix,affinityMatrix,newTarget,groupedNames)
    else:
        return calcMSEWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS)



def calcR2Weights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS):
...
def calcOLSWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS):
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    Y = df[tLabel].copy()
    ...
    return weights,None,None,None,1

def calcNNLSWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS):
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    Y = df[tLabel].copy()
    metaX = pd.DataFrame(columns = list(sourceModels.keys()))
...
def calcRidgeWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS):
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    Y = df[tLabel].copy()
    metaX = pd.DataFrame(columns = list(sourceModels.keys()))
...
def calcOLSFEWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS,distanceMatrix):
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    Y = df[tLabel].copy()
    metaX = pd.DataFrame(columns = list(sourceModels.keys()))
    ...
    totalOLS = targetOLS + sum(sourceOLS.values())
    weights = {'sourceR2s':sourceOLS,'targetR2':targetOLS, 'totalR2':totalOLS, 'metaModel': metaModel,'metaXColumns':metaX.columns}
    return weights,distanceMatrix,None,None,tc

def calcOLSCLWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS):
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    Y = df[tLabel].copy()
    metaX = pd.DataFrame(columns = list(sourceModels.keys()))
    ...
    if len(metaX.columns) >1:
        ...
        k = 7
        groupedNames,distanceMatrix,affinityMatrix,tcPA = STSC(metaX,'euclid',modelID,k,None,None,True)
    ...
    totalOLS = targetOLS + sum(sourceOLS.values())
    weights = {'sourceR2s':sourceOLS,'targetR2':targetOLS, 'totalR2':totalOLS, 'metaModel': metaModel,'metaXColumns':metaX.columns}
    return weights,None,None,None,1#distanceMatrix,affinityMatrix,groupedNames
...
def calcOLSCL2Weights(df,modelSet,targetModel,tLabel,DROP_FIELDS,stableLocals,modelID,PCs,
    ...
    if len(predsDict)>1 and ((groupedNames is None) or newTarget):
        ...
        k=7
        groupedNames,distanceMatrix,affinityMatrix,tcPA = STSC(predsDict,'euclid',modelID,
    ...
    totalOLS = targetOLS + sum(sourceOLS.values())
    weights = {'sourceR2s':sourceOLS,'targetR2':targetOLS, 'totalR2':totalOLS, 'metaModel': metaModel,'metaXColumns':metaX.columns}
    return weights,distanceMatrix,affinityMatrix,groupedNames,(tc,tcPA)
...
def calcOLSPACWeights(df,sourceModels,targetModel,tLabel,DROP_FIELDS,stableLocals,modelID,PCs,
    ...
    if len(modelPCs)>1:
        ...
        k = int(len(modelPCs)/2)
        k=7
        groupedNames,distanceMatrix,affinityMatrix,tcPA = STSC(modelPCs,'principalAngles',modelID,
                k,distanceMatrix,affinityMatrix,newTarget)
        totalPAcalcs+=tcPA
        clusteredNames = [x for x in groupedNames if modelID in x][0]
        # metaX = metaX[metaX.columns[metaX.columns.isin(clusteredNames)]]
    ...
    totalOLS = targetOLS + sum(sourceOLS.values())
    weights = {'sourceR2s':sourceOLS,'targetR2':targetOLS, 'totalR2':totalOLS, 'metaModel': metaModel,'metaXColumns':metaX.columns}
    return weights,distanceMatrix,affinityMatrix,groupedNames,(0,totalPAcalcs)

def calcNewAddExpWeight(df,idx,tLabel,DROP_FIELDS,modelSet,modelID,newModel,weights):
    idx = idx-1
    ...
    return weights,1#,None,None,None#distanceMatrix,affinityMatrix,groupedNames
    

def calcUpdateAddExpWeight(df,idx,modelSet,metaModel,targetModel,modelID,tLabel,DROP_FIELDS):
    idx = idx-1
    X = df.loc[idx].drop(DROP_FIELDS).copy()
    X = X.drop(tLabel).values.reshape(1,-1)
    Y = df.loc[idx,tLabel].copy()
...
def calcOLSAWEWeights(df,modelSet,targetModel,tLabel,DROP_FIELDS,stableLocals,modelID,PCs,
        distanceMatrix=None,affinityMatrix=None,newTarget=False,groupedNames=None):
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    Y = df[tLabel].copy()
    ...
    totalOLS = targetOLS + sum(sourceOLS.values())
    weights = {'sourceR2s':sourceOLS,'targetR2':targetOLS, 'totalR2':totalOLS, 'metaModel': metaModel,'metaXColumns':metaX.columns,'AWEmodelSet':modelSet}
    return weights,None,None,None,tc#distanceMatrix,affinityMatrix,groupedNames


def calcOLSCDOEWeights(df,modelSet,targetModel,tLabel,DROP_FIELDS,stableLocals,modelID,PCs,
        distanceMatrix=None,affinityMatrix=None,newTarget=False,groupedNames=None,metaModelKeep=None):
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    Y = df[tLabel].copy()
    metaX = pd.DataFrame(columns = list(modelSet.keys()))
    ...
    if len(modelSet.keys())>1 and ((groupedNames is None) or newTarget):
        # groupedNames,distanceMatrix,existingBases = STSC(modelPCs,'principalAngles','target',4,distanceMatrix,existingBases)
        # groupedNames,distanceMatrix,affinityMatrix = STSC(modelPCs,'principalAngles',modelID,
                # 7,distanceMatrix,affinityMatrix,newTarget)
        for k,v in modelSet.items():
        ...
        for k,mod in enumerate(metaModelKeep):
            if stableLocals and modelSet:
                if (mod in stableLocals) or (mod in modelSet):
    ...
    totalOLS = targetOLS + sum(sourceOLS.values())
    weights = {'sourceR2s':sourceOLS,'targetR2':targetOLS, 'totalR2':totalOLS, 'metaModel': metaModel,'metaXColumns':metaX.columns}
    return weights,distanceMatrix,affinityMatrix,groupedNames,(tc,totalPAcalcs)
...
def calcOLSFEMIWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS):
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    Y = df[tLabel].copy()
    metaX = pd.DataFrame(columns = list(sourceModels.keys()))
    ...
    totalOLS = targetOLS + sum(sourceOLS.values())
    weights = {'sourceR2s':sourceOLS,'targetR2':targetOLS, 'totalR2':totalOLS, 'metaModel': metaModel,'metaXColumns':metaX.columns}
    return weights,None,None,None,tc

def calcMSEWeights(df,sourceModels,targetModel,modelID,tLabel,DROP_FIELDS):
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    Y = df[tLabel].copy()
    tc=0
...
def updateInitialWeights(df,sourceModels,tLabel,DROP_FIELDS,weightType,ct,mi,distanceMatrix = None, 
    ...
    if weightType == 'R2':
        ...
        return updateInitialOLSFEWeights(df,sourceModels,tLabel,DROP_FIELDS,distanceMatrix)
    elif weightType =='OLSFEMI':
        return updateInitialOLSFEMIWeights(df,sourceModels,tLabel,DROP_FIELDS)
    elif 'OLSCL' in weightType:
        return updateInitialOLSCL2Weights(df,sourceModels,tLabel,DROP_FIELDS,distanceMatrix,affinityMatrix,groupedNames)
    elif weightType == 'MSE':
        return updateInitialMSEWeights(df,sourceModels,tLabel,DROP_FIELDS)
    elif 'PAC' in weightType:
        return updateInitialOLSPACWeights(df,sourceModels,tLabel,DROP_FIELDS,distanceMatrix,affinityMatrix,groupedNames)
    else:
        return updateInitialOLSFEWeights(df,sourceModels,tLabel,DROP_FIELDS,distanceMatrix)
...
def updateInitialOLSFEWeights(df,sourceModels,tLabel,DROP_FIELDS,distanceMatrix):
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    Y = df[tLabel].copy()
    metaX = pd.DataFrame(columns = list(sourceModels.keys()))
...
def updateInitialOLSCL2Weights(df,modelSet,tLabel,DROP_FIELDS,
        distanceMatrix=None,affinityMatrix=None,newTarget=False,groupedNames=None):
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    Y = df[tLabel].copy()
    ...
    k = 7
    if len(predsDict)>1:
        groupedNames,distanceMatrix,affinityMatrix,tcPA = STSC(predsDict,'euclid',None,
                k,distanceMatrix,affinityMatrix,newTarget)
        totalPAcalcs+=tcPA
    ...
    # totalOLS = targetOLS + sum(sourceOLS.values())
    # weights = {'sourceR2s':sourceOLS,'targetR2':targetOLS, 'totalR2':totalOLS, 'metaModel': metaModel,'metaXColumns':metaX.columns}
    return sourceOLS,distanceMatrix,affinityMatrix,groupedNames,(tc,totalPAcalcs)
    

def updateInitialOLSPACWeights(df,sourceModels,tLabel,DROP_FIELDS,distanceMatrix=None,
        affinityMatrix=None,groupedNames=None):
    print(DROP_FIELDS)
    X = df.drop(DROP_FIELDS,axis=1).copy()
    X = X.drop(tLabel,axis=1)
    ...
    if len(modelPCs)>1:
        groupedNames,distanceMatrix,affinityMatrix,tcPA = STSC(modelPCs,'principalAngles',None,
                k,distanceMatrix,affinityMatrix,newTarget)
        totalPAcalcs+=tcPA
...
def initialInstancePredict(df,idx,sourceModels,weights,tLabel,DROP_FIELDS,weightType,ct,mi,distanceMatrix=None,
        affinityMatrix=None,groupedNames=None):
    global CULLTHRESH
    global MITHRESH
    CULLTHRESH = ct
        ...
        if 'metaXColumns' not in weights:
            ...
            if len(historyData)<20:
                ...
                if 'PAC' in weightType or 'CL' in weightType:
                    weights,distanceMatrix,affinityMatrix,groupedNames,tc = updateInitialWeights(historyData,sourceModels,
                            tLabel,DROP_FIELDS,weightType,CULLTHRESH,MITHRESH,distanceMatrix,affinityMatrix,groupedNames)
                else:
                    weights,tc = updateInitialWeights(historyData,sourceModels,tLabel,DROP_FIELDS,weightType,CULLTHRESH,
                            MITHRESH)
    ...
    if 'PAC' in weightType or 'CL' in weightType:
        ...
        return df.loc[idx],distanceMatrix,affinityMatrix,groupedNames,basesSize,bases,tc
    return df.loc[idx],basesSize,bases,tc

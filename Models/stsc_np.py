...
def get_rotation_matrix(X, C):
    ...
    def cost_and_grad(theta_list):
        ...
        cost = np.sum((Z / M) ** 2)
        # cost = np.sum((np.divide(Z, M,out=np.zeros_like(Z), where=M!=0)) ** 2)
        # cost = np.sum((np.divide(Z, M,out=np.full_like(Z,np.inf), where=M!=0)) ** 2)
        grad = np.zeros(K)
        for k in range(K):
            A = get_A_matrix(X, U_list, V_list, k, K)
            tmp = (Z / (M ** 2)) * A
            tmp -= ((Z ** 2) / (M ** 3)) * (np.choose(mi, A.T).reshape(-1, 1))
            # tmp = (np.divide(Z,(M ** 2),out=np.zeros_like(Z), where=M!=0)) * A
            # tmp -= (np.divide((Z ** 2),(M ** 3),out=np.zeros_like(Z),where=M!=0)) * (np.choose(mi, A.T).reshape(-1, 1))
            # tmp = (np.divide(Z,(M ** 2),out=np.full_like(Z,np.inf), where=M!=0)) * A
            # tmp -= (np.divide((Z ** 2),(M ** 3),out=np.full_like(Z,np.inf),where=M!=0)) * (np.choose(mi, A.T).reshape(-1, 1))
            tmp = 2 * np.sum(tmp)
            grad[k] = tmp
